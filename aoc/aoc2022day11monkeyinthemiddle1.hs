...
inspectFirstAt :: Int -> Seq Monkey -> Seq Monkey
inspectFirstAt idx ms =
  ...
  where
    ...
    new = runOp (op monkey) item `div` 3
    toM = if new `mod` divTest monkey == 0
      then mTrue monkey
      else mFalse monkey
    addItem x m = m { items = items m Seq.|> x }

inspectAllAt :: Int -> Seq Monkey -> Seq Monkey
inspectAllAt idx ms
  | Seq.null (items m) = ms
  | otherwise          = inspectAllAt idx $ inspectFirstAt idx ms
  where m = ms `Seq.index` idx

inspectRound :: Seq Monkey -> Seq Monkey
inspectRound ms = snd $ (!! Seq.length ms) $ flip iterate (0, ms)
  $ \(i, ms) -> (i+1, inspectAllAt i ms)

solve :: Seq Monkey -> Int
solve ms = product $ Seq.take 2 $ Seq.reverse $ Seq.sort
  $ fmap inspected $ iterate inspectRound ms !! 20
